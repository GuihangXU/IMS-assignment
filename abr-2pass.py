# This is code for encoding video using 2-Pass ABR (average fixed bitrate) model, and calculated the PSNR, SSIM and VMAF of the encoded videos
...
# 2-pass video generation, vmaf ssim psnr
		print '2-pass ABR: bitrate ', bitrate, '\n'

		
		net="./ffmpeg -y -i \""+filepath+filename+rawformat+"\" -c:v libx264 -b:v "+bitrate+" -pass 1 -f mp4 /dev/null && ./ffmpeg -i \""+filepath+filename+rawformat+"\" -c:v libx264 -b:v "+bitrate+" -pass 2 \""+filepath+"output_videos/"+filename+"_2pass_"+bitrate+encformat+"\""
	...
	# generate psnr.log & reportpsnr.log in /output_logs/
		netpsnr="./ffmpeg -i \"" +filepath+"output_videos/"+filename+"_2pass_"+bitrate+encformat+"\" -i \""+filepath+filename+rawformat+"\" -lavfi psnr=\""+filepath+"output_logs/psnr_"+filename+"_2pass_"+bitrate+logformat+"\" -f null - 2> \""+ filepath+"output_logs/reportpsnr_"+filename+"_2pass_"+bitrate+logformat+"\""
		print(netpsnr), '\n'
		os.system(netpsnr)
		print 'bitrate ', bitrate, ' psnr calculation success!'	

	# generate ssim.log & reportssim.log in /output_logs/
		netssim="./ffmpeg -i \"" +filepath+"output_videos/"+filename+"_2pass_"+bitrate+encformat+"\" -i \""+filepath+filename+rawformat+"\" -lavfi ssim=\""+filepath+"output_logs/ssim_"+filename+"_2pass_"+bitrate+logformat+"\" -f null - 2> \""+ filepath+"output_logs/reportssim_"+filename+"_2pass_"+bitrate+logformat+"\""
		print(netssim), '\n'
		os.system(netssim)
		print 'bitrate ', bitrate, ' ssim calculation success!'

	# generate vmaf.log & reportvmaf.log in /output_logs/
		netvmaf="./ffmpeg -i \"" +filepath+"output_videos/"+filename+"_2pass_"+bitrate+encformat+"\" -i \""+filepath+filename+rawformat+"\" -lavfi libvmaf=\"model_path=./model/vmaf_v0.6.1.pkl:psnr=1:log_fmt=json:log_path="+filepath+"output_logs/vmaf_"+filename+"_2pass_"+bitrate+".json\" -f null - 2> \""+ filepath+"output_logs/reportvmaf_"+filename+"_2pass_"+bitrate+logformat+"\""
		print(netvmaf), '\n'
		os.system(netvmaf)
